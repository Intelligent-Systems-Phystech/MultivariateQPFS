...
{\color{red}Here, a subset of features defines a forecasting model.}
...
This approach was proposed and investigated in~\citep{ding2005minimum,yamada2014high}.
 {\color{red} It solves the optimization problem to select an optimal feature set.} The paper~\citep{katrutsa2017comprehensive} shows that the QPFS algorithm outperforms many existing feature selection methods for the univariate regression problem.  {\color{red} The admissible feature set is denoted in~\eqref{eq:qpfs_problem} by the vector~$\mathbf{z}$. Each selected feature is indicated by one element of this vector, which makes~$2^n$-1 admissible combinations of features. The solution~$\mathbf{z}$ of the quadratic form~\eqref{eq:qpfs_problem} is delivered by a convex optimization algorithm with relaxation of the integer variable, here~$\mathbf{z}\in\mathbb{R}^n$. The algorithm solves the following optimization problem:}
\begin{equation}
...
The columns of matrix~$\bX$ are the features, and~$\bY$ %\bnu$ 
is the target. 
...
To measure the similarity, the authors of~\citep{rodriguez2010quadratic} used the absolute value of the sample correlation coefficient between pairs of features for the Sim function, and between the features and the target vector for the Rel function. {\color{red} The detailed explanation of this optimization problem is given in next section.}
...
We propose methods that take into account the dependencies in both input and target spaces.  It allows to form a stable sparse model.
We refer to the original QPFS algorithm as our baseline for the computational experiment. {\color{red} The convergence of the quadratic programming algorithms were shown theoretically in~\citep{isachenko2018quadratic}. The class of convex algorithms was investigated by~\citep{nesterov1983momentum} and~\citep{blaschke1996some}.}

The main drawback of the QPFS algorithm is its computational costs. However, the original paper~\citep{rodriguez2010quadratic} suggests a way to solve the quadratic problem~\eqref{eq:qpfs_problem} efficiently. Additionally, in~\citep{prasad2013scaling} the proposed sequential minimal optimization framework solves the problem~\eqref{eq:qpfs_problem}. {\color{red} There are alternative ways for solving such optimization problems~\citep{preitl2006use,chiang2014apn,li2020hard,precup2021optimal}. Our choice of the QPFS formulation is based on the applicability of this approach to the ECOG data~\citep{motrenko2018multi}, its success on the feature selection task~\citep{katrutsa2017comprehensive}, and the interpretability of the solution in terms of linear multicorrelation.}

The experiments were carried out using the ECoG dataset~\citep{shimoda2012decoding}.
We compared the proposed methods for multivariate feature selection with the baseline strategy and the PLS algorithm~\citep{isachenko2018plsdestech}. 
...
\end{equation*}
where subscript~$\ba$ indicates the sub matrix with the columns in which the components of~$\ba$ equal 1. {\color{red} Further in this section we derive the proposed feature selection methods.}
...
{\color{red} All convex quadratic optimization problems had been solved using CVXPY open-source Python library~\citep{diamond2016cvxpy,agrawal2018rewriting}. The source code for the paper could be found here~\footnote{\href{https://github.com/Intelligent-Systems-Phystech/MultivariateQPFS}{https://github.com/Intelligent-Systems-Phystech/MultivariateQPFS}}}.

\subsection{Data}

We conducted a computational experiment with the ECoG data from the NeuroTycho project~\citep{shimoda2012decoding,motrenko2018multi}. {\color{red} This data and the problem of Brain-Computer Interface construction itself demanded the proposed theory due to the high multicorrelation and redundancy of the features. The paper ~\citep{motrenko2018multi} shows experimentally three types of multicorrelated data. First, the ECoG signals are cross-correlated due the short distance between the cortical electrodes. Second, the limb movement sequence is interdependent due its physical nature. And third, there is the multiple correlation between the brain and limb signals, which established is the main reason to construct the BCI forecasting models.}

The ECoG data consist of brain voltage signals recorded over 32 channels.
The goal is to predict 3D hand positions in subsequent moments given the input signal. The initial voltage signals are transformed to the spatial-temporal representation using the wavelet transformation with the Morlet mother wavelet.
...
We unfold the data and feature description at each time moment has dimension of size $32\text{\,(channels)} \times 27 \text{\,(frequencies)} = 864$.
...
\begin{figure}[h]
	\begin{minipage}{.5\linewidth}
		\subfloat{
			\includegraphics[width=\linewidth]{features_vs_alpha_ecog_3.pdf}}
	\end{minipage}%
	\begin{minipage}{.5\linewidth}
		\subfloat{
			\includegraphics[width=\linewidth]{features_vs_alpha_ecog_6.pdf}}
	\end{minipage}\par\medskip
	\subfloat{
		\includegraphics[width=\linewidth]{features_vs_alpha_ecog_9.pdf}}
...
\begin{figure}[h]
	\includegraphics[width=\linewidth]{ecog_3_30_metrics.pdf}
